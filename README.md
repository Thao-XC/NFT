...
<title>Thao's Fortune Telling Shop</title>
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .brush{
    ...
    line-height:1;
    margin-bottom:4px;
    text-shadow:0 0 30px rgba(184,147,90,0.25);
  }
  h1{
  ...
  .card{
    ...
    border-radius:4px;
    padding:32px 30px;
    box-shadow:0 30px 60px -20px rgba(0,0,0,0.6), 0 0 0 1px rgba(184,147,90,0.15);
    position:relative;
  }
  .card::before{
    content:"";
    position:absolute; inset:8px;
    border:1px solid rgba(163,56,42,0.25);
    pointer-events:none;
  }
  label{
    display:block;
  ...
  .num-row{display:flex; gap:16px; margin-bottom:24px;}
  .num-field{flex:1;}
  .num-control{
  ...
  .num-control input{
    ...
    text-align:center; padding:10px 4px; color:var(--ink-text);
    -moz-appearance:textfield;
  }
  .num-control input::-webkit-outer-spin-button,
  .num-control input::-webkit-inner-spin-button{-webkit-appearance:none; margin:0;}
  .num-control input:focus{outline:none;}
  .num-btn{
    border:none; background:transparent; color:var(--seal);
    font-size:16px; width:34px; cursor:pointer;
    font-family:'JetBrains Mono', monospace;
  }
  .num-btn:hover{background:rgba(163,56,42,0.08);}
  .dice-btn{
    border:none; background:transparent; color:#8a8070;
    width:34px; cursor:pointer; font-size:15px; border-left:1px solid var(--paper-dim);
  }
  ...
  .cast-btn:active{transform:scale(0.98);}
  .cast-btn:disabled{opacity:0.6; cursor:default;}

  .hint{font-size:12px; color:#948a76; margin:-14px 0 22px; line-height:1.5;}

  #result{margin-top:32px; display:none;}
  .hexline-wrap{display:flex; justify-content:center; margin-bottom:22px;}
  .hexlines{display:flex; flex-direction:column-reverse; gap:7px;}
  .hexline{width:120px; height:9px; position:relative;}
  .hexline .bar{position:absolute; top:0; bottom:0; background:var(--ink-text); border-radius:1px;}
  .hexline.solid .bar{left:0; right:0;}
  .hexline.broken .bar1{left:0; width:44%;}
  .hexline.broken .bar2{right:0; width:44%;}
  .hexline.active .bar{background:var(--seal);}
  .hexline.active::after{
    content:"\25c0"; position:absolute; left:132px; top:-3px;
  ...
  .que-sub{text-align:center; font-size:12px; letter-spacing:0.1em; color:#8a8070; text-transform:uppercase; margin-bottom:22px;}

  .section{margin-bottom:18px;}
  .section-label{font-size:11px; text-transform:uppercase; letter-spacing:0.1em; color:var(--seal); font-weight:600; margin-bottom:6px;}
  .plain-text{font-size:15.5px; line-height:1.55;}
  .classical-toggle{
    background:none; border:none; color:#8a8070; font-size:12px;
  ...
  .classical-text{
    ...
    color:#5c5346; margin-top:8px; display:none; line-height:1.5;
    border-left:2px solid var(--paper-dim); padding-left:12px;
  }
  .classical-text.show{display:block;}

  .verdict-wrap{display:flex; justify-content:center; margin:26px 0 20px;}
  .stamp{
    ...
    animation:stamp-press .45s cubic-bezier(.2,1.6,.4,1) forwards;
    animation-delay:.05s;
  }
  .stamp.mixed{border-color:var(--gold); color:var(--gold);}
  ...
  .bottom-line{
    background:#f6f1e6; border-left:3px solid var(--gold);
    padding:14px 16px; font-size:15px; line-height:1.5; border-radius:2px;
  }
  .bottom-line b{color:var(--seal-dark);}

  .loading{display:none; text-align:center; padding:20px 0; font-family:'Cormorant Garamond', serif; font-style:italic; color:#8a8070; font-size:15px;}

  footer{text-align:center; font-size:11px; color:#4d4860; margin-top:28px; line-height:1.6;}
  footer a{color:#7a7490;}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="card">
    <label for="question">Your question <span style="font-weight:400; text-transform:none; color:#a89c85;">(optional)</span></label>
    <input type="text" id="question" placeholder="e.g. Should I take the new job offer?">
    ...
    <button class="cast-btn" id="castBtn" onclick="cast()">Cast the Reading</button>
    <div class="loading" id="loading">Consulting the que&hellip;</div>

    <div id="result">
      <div class="hexline-wrap"><div class="hexlines" id="hexlines"></div></div>
      <div class="que-title" id="queTitle"></div>
      <div class="que-sub" id="queSub"></div>

      <div class="section">
        <div class="section-label">The Judgment</div>
        <div class="plain-text" id="judgmentPlain"></div>
        <button class="classical-toggle" onclick="toggleClassical(this)">show classical wording</button>
        <div class="classical-text" id="judgmentClassical"></div>
      </div>

      <div class="section">
        <div class="section-label" id="haoLabel">Your Line</div>
        <div class="plain-text" id="linePlain"></div>
        <button class="classical-toggle" onclick="toggleClassical(this)">show classical wording</button>
        <div class="classical-text" id="lineClassical"></div>
      </div>
...
function randomize(id, max){
  document.getElementById(id).value = Math.floor(Math.random()*max)+1;
}

function modernize(text){
  let result = text;
  for(const [oldPhrase, newPhrase] of DATA.GLOSSARY){
    const re = new RegExp(oldPhrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'ig');
    result = result.replace(re, newPhrase);
  }
  return result;
}
...
function detectTheme(question){
  ...
  for(const key in DATA.THEMES){
    ...
    for(const kw of t.keywords){
      if(kw.includes(' ')){
        if(lowered.includes(kw)) return t;
      } else if(words.includes(kw)){
        return t;
      }
    }
  }
  return {topic:'your question', good:'the signs point toward moving forward', bad:'the signs point toward waiting'};
}

// Figures out what KIND of question this is, since "why", "when", and
// "how" questions can't honestly be answered with a plain yes/no.
function detectQuestionType(question){
  const q = question.trim().toLowerCase();
  if(/^when\b/.test(q)) return 'when';
  if(/^why\b/.test(q)) return 'why';
  if(/^how\b/.test(q)) return 'how';
  if(/^(will|is|are|does|do|did|was|were|can|should|shall)\b/.test(q)) return 'yesno';
  return 'other';
}

function scoreFortune(judgment, line){
  const text = (judgment + ' ' + line).toLowerCase();
  let pos = 0, neg = 0;
  for(const w of DATA.POSITIVE_WORDS){ pos += text.split(w).length - 1; }
  for(const w of DATA.NEGATIVE_WORDS){ neg += text.split(w).length - 1; }
  return [pos, neg];
}

function toggleClassical(btn){
  const box = btn.nextElementSibling;
  box.classList.toggle('show');
  btn.textContent = box.classList.contains('show') ? 'hide classical wording' : 'show classical wording';
}

function buildHexlines(que, hao){
  const wrap = document.getElementById('hexlines');
  wrap.innerHTML = '';
  // Stylized line pattern seeded from que number (not the authentic binary
  // encoding - a real King Wen lookup table would be needed for that).
  ...
  for(let i=1; i<=6; i++){
    ...
    if(solid){
      ...
      div.innerHTML = '<div class="bar bar1"></div><div class="bar bar2"></div>';
    }
...
function cast(){
  ...
  if(!que || que < 1 || que > 64 || !hao || hao < 1 || hao > 6){
    alert('Que must be 1-64 and Hao must be 1-6.');
    return;
  }
  ...
  setTimeout(() => {
    const hex = DATA.OFFLINE_DATA[String(que)];
    ...
    document.getElementById('queSub').textContent = question ? '"' + question + '"' : 'no question asked';
    document.getElementById('haoLabel').textContent = 'Hao ' + hao + ' (your specific line)';

    document.getElementById('judgmentPlain').textContent = modernize(judgment);
    document.getElementById('judgmentClassical').textContent = judgment;
    document.getElementById('lineClassical').textContent = line;
    document.getElementById('linePlain').textContent = modernize(line);

    document.querySelectorAll('.classical-text').forEach(el => el.classList.remove('show'));
    document.querySelectorAll('.classical-toggle').forEach(el => el.textContent = 'show classical wording');

    buildHexlines(que, hao);

    const stamp = document.getElementById('stamp');
    const bottomLine = document.getElementById('bottomLine');
    stamp.classList.remove('mixed');
    stamp.style.animation = 'none'; stamp.offsetHeight; stamp.style.animation = null;

    if(question){
      const [pos, neg] = scoreFortune(judgment, line);
      const favorable = pos > neg;
      const mixed = pos === neg;
      ...
      if(qType === 'when'){
        headline = mixed ? 'UNCLEAR' : (favorable ? 'SOON' : 'NOT YET');
        advice = mixed
          ? "the timing isn't clearly signaled - reassess in a few days"
          : (favorable ? "sooner rather than later looks right" : "it's better to let more time pass first");
        if(!favorable || mixed) stamp.classList.add('mixed');
      } else if(qType === 'why'){
        headline = 'REFLECT';
        advice = mixed
          ? "the reading is mixed - the reason may be more than one thing"
          : (favorable
              ? "the reason likely has to do with something currently working in your favor"
              : "the reason may be that something's out of balance or being rushed");
        stamp.classList.add('mixed');
      } else if(qType === 'how'){
        headline = mixed ? 'EITHER WAY' : (favorable ? 'DIRECTLY' : 'CAREFULLY');
        advice = mixed
          ? "either a direct or careful approach could work - go with what feels natural"
          : (favorable ? "a direct, confident approach works best right now" : "a slower, more careful approach serves you better right now");
        if(!favorable || mixed) stamp.classList.add('mixed');
      } else {
        // "will", "should", "is", "does", or anything else - treat as yes/no
        if(favorable){ headline = 'YES'; advice = theme.good; }
        else if(!mixed){ headline = 'NO'; advice = theme.bad; stamp.classList.add('mixed'); }
        else { headline = 'UNCLEAR'; advice = "the signs don't strongly favor either way - trust your gut"; stamp.classList.add('mixed'); }
      }

      stamp.textContent = headline;
      bottomLine.innerHTML = '<b>Bottom line:</b> ' + advice + '.';
    } else {
      stamp.textContent = 'READ';
      bottomLine.innerHTML = 'Add a question above next time for a direct, tailored take.';
    }
